...
namespace AudioVisualizationLibrary.Classes.ColorLibrary
{
    ...
    public static class ColorTranslator
    {
        ...
        /// </summary>
        private static readonly double Tolerance = 0.000000000000001;
        ...
        /// 
        /// </summary>
        /// <param name="h">0 - 360 arası bir değer giriniz.</param>
        /// <param name="s">0 - 1 arası bir değer giriniz.</param>
        /// <param name="l">0 - 1 arası bir değer giriniz.</param>
        /// <param name="a">0 - 255 arası bir değer giriniz.</param>
        /// <returns></returns>
        public static Color HsLtoRgb(double h, double s, double l, byte a = 255)
        {
            return HsLtoRgb(h, s, l, (int)a);
        }
                ...
                byte? outputAlpha = null)
        {
            double[] hsl = colorTransformMode == ColorTransformMode.Hsl
                    ? ColorTranslator.RgBtoHsl(color)
                    : ColorTranslator.RgBtoHsb(color);
            if ((Math.Abs(hsl[2]) < Tolerance)
            ...
            return colorTransformMode == ColorTransformMode.Hsl
                    ? ColorTranslator.HsLtoRgb(hsl[0], hsl[1], hsl[2], outputAlpha ?? color.A)
                    : ColorTranslator.HsBtoRgb(hsl[0], hsl[1], hsl[2], outputAlpha ?? color.A);
        }
